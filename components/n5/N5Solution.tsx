...
export default function N5Solution() {
    ...
    <section ref={ref} className="py-16 md:py-24 bg-[#080808]">
      <div className="max-w-3xl mx-auto px-5">
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ delay: 0.3 }}
          ...
          <p className="text-white/40 text-lg">שיעורים קצרים של 2–5 דקות. מושלם לצפייה בדרך לעבודה או ללימודים.</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          {STAGES.map((stage, i) => (
            <div key={i} className="rounded-2xl overflow-hidden border p-4 xs:p-6 md:p-8"
              ...
              <p className="text-white/60 text-lg">{stage.about}</p>
            </div>
          ))}
        </motion.div>
