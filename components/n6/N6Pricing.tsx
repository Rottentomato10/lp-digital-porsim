...
export default function N6Pricing() {
  ...
  return (
    <section id="pricing" ref={ref} className="relative py-20 md:py-28 bg-[#0F1520] overflow-hidden">
      <div className="pointer-events-none absolute inset-0"
        style={{ background: 'radial-gradient(ellipse 80% 60% at 50% 40%, rgba(245,166,36,0.04) 0%, transparent 65%)' }} />
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-5">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          className="rounded-3xl border border-[#F5A624]/20 bg-[#141E2B] overflow-hidden"
          style={{ boxShadow: '0 0 60px rgba(245,166,36,0.05)' }}>

          <div className="h-0.5 bg-gradient-to-r from-transparent via-[#F5A624] to-transparent" />

          {/* Launch discount banner */}
          <div className="bg-[#F5A624]/8 border-b border-[#F5A624]/12 px-5 py-3 text-center">
          ...
          <div className="p-5 xs:p-8 md:p-12">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-white/35 text-base">{contentD.pricing.price_note}</p>
            </div>

            {/* ROI line */}
            <p className="text-center text-white/35 text-base md:text-lg mb-4">
              ...
              <div className="space-y-4">
                {VALUE_ITEMS.map((item, i) => (
                  <div key={i}>
                    ...
                    {item.desc && (
                      <p className="text-white/25 text-sm mr-6 mt-1">{item.desc}</p>
                    )}
                  </div>
                ))}
              </div>
              <div className="mt-4 pt-4 border-t border-white/8 flex items-center justify-between">
                <span className="text-white/35 text-sm">שווי כולל</span>
                <span className="text-white/35 text-base font-semibold line-through">₪{totalValue}</span>
              </div>
              <div className="mt-2 flex items-center justify-between">
                <span className="text-[#F5A624]/70 text-sm font-bold">אתה משלם עכשיו</span>
                <span className="text-[#F5A624] text-base font-black">₪{contentD.pricing.price}</span>
              </div>
            </div>

            {/* Guarantee */}
            <div className="p-5 xs:p-6 md:p-8 rounded-2xl border border-[#F5A624]/20 bg-[#F5A624]/5">
              ...
              <div className="space-y-4 text-white/55 text-base md:text-lg leading-relaxed">
