...
export default function N6Reviews() {
    ...
    <section ref={ref} className="py-16 md:py-24 bg-[#0F1520]">
      <div className="max-w-4xl mx-auto px-5">
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} className="text-center mb-10">
          <span className="text-[#F5A624] font-semibold text-sm tracking-widest uppercase">מה אומרים הבוגרים</span>
        </motion.div>

        {/* Featured review */}
        <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.7 }}
          className="relative p-5 xs:p-8 md:p-10 rounded-3xl border border-[#F5A624]/15 bg-[#141E2B] mb-6 overflow-hidden text-center">
          <div className="pointer-events-none absolute inset-0"
            style={{ background: 'radial-gradient(ellipse 70% 70% at 50% 100%, rgba(245,166,36,0.05), transparent)' }} />
            ...
            <div className="flex flex-col items-center gap-1">
              <div className="flex items-center gap-2">
                ...
                <div className="text-right">
                  <p className="text-white/45 text-sm">{FEATURED.name} · {FEATURED.city}</p>
                  <div className="flex items-center gap-2">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          {REVIEWS.map((review, i) => (
            <div key={i} className="p-6 rounded-2xl bg-[#1A2333] border border-white/7">
              <Stars />
              <p className="text-white/60 text-lg leading-relaxed mb-4">״{review.quote}״</p>
              <div className="flex items-center gap-2">
                <Avatar name={review.name} color={AVATAR_COLORS[i % AVATAR_COLORS.length]} />
                <div>
                  <p className="text-white/45 text-sm">{review.name} · {review.city}</p>
                  <div className="flex items-center gap-2">
